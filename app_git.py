...
# Version: 1.8 — Cloud-Stable (HuggingFace Integrated)
# ===============================================
...
st.markdown(
    "<small style='color:gray;'>🎤 If the microphone doesn’t start, refresh and allow microphone access.</small>",
    unsafe_allow_html=True
)

# --- Choose compute type dynamically ---
...
if "darwin" in system.lower() and "apple" in processor:
    ...
    compute_type = "int8_float16"  # Colab / Cloud
...
if wav_audio_data:
    ...
    if recognized_text.strip():
        ...
        st.caption(f"⚡ {end - start:.2f}s processing ({info.duration:.2f}s audio)")
    else:
